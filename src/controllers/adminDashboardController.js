...
const getDashboardStats = async (req, res) => {
  try {
    ...
    // Get recent withdrawal requests (cashout requests) with real data
    const recentWithdrawals = await pool.query(`
      SELECT wr.id, wr.amount, wr.status, wr.created_at, wr.admin_notes,
    ...
    res.json({
      ...
      stats: {
        ...
        onboardedUsers: parseInt(onboardedUsersResult.rows[0]?.onboarded_users || 0),
        pendingUsers: parseInt(pendingUsersResult.rows[0]?.pending_users || 0),
        totalOrders: parseInt(ordersResult.rows[0]?.total_orders || 0),
        totalSales: parseFloat(ordersResult.rows[0]?.total_sales || 0),
        totalRevenue: parseFloat(revenueResult.rows[0]?.total_revenue || 0),
        totalProducts: parseInt(productsResult.rows[0]?.total_products || 0),
        todayEarnings: parseFloat(earningsResult.rows[0]?.today_earnings || 0),
        pendingWithdrawals: parseInt(withdrawalRequestsResult.rows[0]?.pending_withdrawals || 0),
        totalWithdrawalAmount: parseFloat(withdrawalRequestsResult.rows[0]?.total_withdrawal_amount || 0),
        pendingPayments: parseInt(pendingPaymentsResult.rows[0]?.pending_payments || 0),
        totalWalletBalance: parseFloat(totalWalletBalanceResult.rows[0]?.total_wallet_balance || 0),
        totalMlmEarnings: parseFloat(mlmEarningsResult.rows[0]?.total_mlm_earnings || 0),
        totalWithdrawals: parseFloat(totalWithdrawalsResult.rows[0]?.total_withdrawals || 0),
        mlmWithdrawals: parseFloat(mlmWithdrawalsResult.rows[0]?.mlm_withdrawals || 0),
        netMlmEarnings: parseFloat(mlmEarningsResult.rows[0]?.total_mlm_earnings || 0) - parseFloat(mlmWithdrawalsResult.rows[0]?.mlm_withdrawals || 0)
      },
